...
const FormBattle = ({ submitPropsFunction, initialBattle = false }) => {
    ...
    const setTeam = (teamNumber) => initialBattle.Participations.filter(element =>
            element.team === teamNumber).map(element => ({ value: `${element.character_id}` }));
    ...
    return (
        <form className="card large" onSubmit={handleSubmit}>
            ...
            <div className="form-grid">
                ...
                <div className="label-input teamA">
                    <label htmlFor="teamA">Team Rouge :</label>
                ...
                <div className="label-input teamB">
                    <label htmlFor="teamB">Team Bleu :</label>
            ...
            <button className="btn-primary btn-large" type="submit">{!initialBattle ? <span>Créer le combat</span> : <span>Modifier le combat</span>}</button>
        </form>
    );
};
